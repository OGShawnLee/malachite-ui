...
describe.skip('generate', () => {
	...
	it.skip('Should return an array of the given length filled with the given value', () => {
	...
	describe.skip('function value', () => {
		it.skip('Should fill the array with the return value', () => {
			const names = generate(5, () => 'James');
			expect(names).toHaveLength(5);
			expect(isArray(names, (name): name is string => name === 'James')).toBe(true);
		});

		it.skip('Should call the function as many times as the given length', () => {
			const func = vi.fn(() => {});
			generate(10, func);
			expect(func).toBeCalledTimes(10);
		});

		it.skip('Should pass the current index', () => {
			const func = vi.fn(() => {});
			generate(5, func);
...
describe.skip('getFirstAndLast', () => {
	const { getFirstAndLast } = array;
	const items = [0, 1, 2, 3];

	it.skip('Should return the first and last in a tuple', () => {
		expect(getFirstAndLast(items)).toBeInstanceOf(Array);
		expect(getFirstAndLast(items)).toHaveLength(2);
	});

	it.skip('Should return the first and last item', () => {
		expect(getFirstAndLast(items)).toEqual([0, 3]);
	});

	it.skip('Should return the first item and undefined if the array has only one item', () => {
		expect(getFirstAndLast([10])).toEqual([10, undefined]);
	});
});

describe.skip('makeUnique', () => {
	it.skip('Should return an array with unique items', () => {
